...
import AssyncTasks.GetSedadla;
import AssyncTasks.SaveTicket;
import AssyncTasks.deleteBenefit;
import AssyncTasks.getFromTO;
import AssyncTasks.getPouzID;
...
public class Sedadla64 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		index = Listok.zoznam.get(0).getIndex();
		normal = Listok.zoznam.get(0).getNormal();
		student = Listok.zoznam.get(0).getStudent();
		ztp = Listok.zoznam.get(0).getZtp();
		dochod = Listok.zoznam.get(0).getDochod();
		dovolene = normal + student + ztp + dochod;
		String start = Rezervacia_cas.spoje.get(index).getStart();
		String finish = Rezervacia_cas.spoje.get(index).getCiel();
		new getFromTO(this, this).execute(start, finish);
		Log.e("dovolene", Integer.toString(dovolene));
		id = Rezervacia_cas.spoje.get(index).getId();
		String zaciatok = Rezervacia_cas.spoje.get(index).getOdchod();
		Log.e("Odchod cas", zaciatok);
		new GetSedadla(this, this).execute(Integer.toString(id),
		...
		new getPouzID(this, this).execute(meno);
	...
	public void putID(Integer id) {
		this.ID = id;
		Log.e("ID pouz", Integer.toString(ID));
	}

	private void NaplnenieListka() {
		DecimalFormat df = new DecimalFormat("#.##");
		String jazda = Integer
				.toString(Rezervacia_cas.spoje.get(index).getId());
		// tu ma byt for a vklada sa typ listka zvlast a miesto
		int a = 0, b = 0, c = 0, d = 0, dokopy = 0;
		Log.e("NaplnenieListka", "spustilo sa");
		for (a = 0; a < normal; a++) {
			// ukladaj normalne
			Log.e("NaplnenieListka", "spustilo sa vkladanie normalneho");
			String miesto = Integer.toString(zvolene_sedadla.get(dokopy));
			new SaveTicket(this).execute(Integer.toString(ID), jazda, "1", z,
					ciel, miesto, Rezervacia_cas.spoje.get(Listok.index)
							.getOdchod(), df.format(Listok.c_normal), df
							.format(Listok.b_normal), VolbaJazdy.prichod);
			dokopy++;
		}
		for (b = 0; b < student; b++) {
			// ukladaj student
			String miesto = Integer.toString(zvolene_sedadla.get(dokopy));
			new SaveTicket(this).execute(Integer.toString(ID), jazda, "2", z,
					ciel, miesto, Rezervacia_cas.spoje.get(Listok.index)
							.getOdchod(), df.format(Listok.c_student), df
							.format(Listok.b_student), VolbaJazdy.prichod);
			dokopy++;
		}
		for (c = 0; c < dochod; c++) {
			// ukladaj dochodkove
			String miesto = Integer.toString(zvolene_sedadla.get(dokopy));
			new SaveTicket(this).execute(Integer.toString(ID), jazda, "3", z,
					ciel, miesto, Rezervacia_cas.spoje.get(Listok.index)
							.getOdchod(), df.format(Listok.c_dochodca), df
							.format(Listok.b_dochodca), VolbaJazdy.prichod);
			dokopy++;
		}
		for (d = 0; d < ztp; d++) {
			// ukladaj ztp
			String miesto = Integer.toString(zvolene_sedadla.get(dokopy));
			new SaveTicket(this).execute(Integer.toString(ID), jazda, "4", z,
					ciel, miesto, Rezervacia_cas.spoje.get(Listok.index)
							.getOdchod(), df.format(Listok.c_ztp), df
							.format(Listok.b_ztp), VolbaJazdy.prichod);
			dokopy++;
		}

		new AddBenefit(this).execute(df.format(Listok.benefit),
				Integer.toString(ID));
		if (Listok.stlacene == true) {
			new deleteBenefit(this).execute(Integer.toString(ID),
					Float.toString(Listok.benefit_suma));
		}

	}

	public void putTowns(String start, String ciel) {
